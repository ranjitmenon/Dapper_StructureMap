...
namespace DapperExtension
{
    ...
    public static partial class SimpleCRUD
    {
        ...
        public static bool InsertList<T>(this IDbConnection connection, IEnumerable<T> entitiesToInsert, IDbTransaction transaction = null, int? commandTimeout = null)
        {
            var idProps = GetIdProperties(entitiesToInsert.FirstOrDefault()).ToList();
            ...
            var name = GetTableName(entitiesToInsert.FirstOrDefault());
            var sb = new StringBuilder();
            foreach (var entity in entitiesToInsert)
            {
                ...
                sb.Append("values");
                sb.Append(" (");
                BuildInsertWithValues(entity, sb);
                sb.Append("); ");
            }
            ...
            var affectedRows = connection.Execute(sb.ToString(), null, transaction);
            return affectedRows == entitiesToInsert.Count();
        }
        ...
        /// <param name="commandTimeout"></param>
        /// <returns>The number of effected records</returns>
        public static bool UpdateList<T>(this IDbConnection connection, IEnumerable<T> entities, object parameters = null, IDbTransaction transaction = null, int? commandTimeout = null)
        {
            var idProps = GetIdProperties(entities.FirstOrDefault()).ToList();

            if (!idProps.Any())
                throw new ArgumentException("Entity must have at least one [Key] or Id property");

            var name = GetTableName(entities.FirstOrDefault());
            var sb = new StringBuilder();
            foreach (var entity in entities)
            {
                ...
                sb.Append(" where ");
                BuildWhereWithValue(sb, idProps, entity);
                sb.Append("; ");
            }
            ...
            var affectedRows = connection.Execute(sb.ToString(), parameters, transaction);
        ...
        //build update statement based on list on an entity
        private static void BuildUpdateSetWithValue(object entityToUpdate, StringBuilder sb)
        {
            var nonIdProps = GetUpdateableProperties(entityToUpdate).ToArray();
            ...
            for (var i = 0; i < nonIdProps.Length; i++)
            {
                ...
                sb.AppendFormat("{0} = {1}", GetColumnName(property), ValueWrapper(property.GetValue(entityToUpdate)));
                if (i < nonIdProps.Length - 1)
                    sb.AppendFormat(", ");
            }
        }
        ...
        private static void BuildWhereWithValue(StringBuilder sb, IEnumerable<PropertyInfo> idProps, object sourceEntity, object whereConditions = null)
        {
            var propertyInfos = idProps.ToArray();
            for (var i = 0; i < propertyInfos.Length; i++)
            {
                ...
                sb.AppendFormat(
                    useIsNull ? "{0} is null" : "{0} = {1}",
                    GetColumnName(propertyToUse),
                    ValueWrapper(propertyInfos.ElementAt(i).GetValue(sourceEntity)));
        ...
        private static void BuildInsertWithValues(object entityToInsert, StringBuilder sb)
        {
            var props = GetScaffoldableProperties(entityToInsert).ToArray();
            for (var i = 0; i < props.Length; i++)
            {
                ...
                sb.AppendFormat("{0}", ValueWrapper(property.GetValue(entityToInsert)));
                if (i < props.Length - 1)
                    sb.Append(", ");
            }
            if (sb.ToString().EndsWith(", "))
